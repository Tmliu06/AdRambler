...
class Handlers
{
	static var trafficdir = "D://traffic/";
    // ip_pkg.txt contains the mapping between device_ip and the app being tested
	static var pkgpath = trafficdir + "ip_pkg.txt";
    ...
    */
    ...
    static function OnBeforeRequest(oSession: Session) {
        // "x-OverrideGateway" is for fiddler proxy setting
		...
		{
			var ip = oSession.m_clientIP.Split(":");
			var dir0 = trafficdir + ip[ip.Length-1] + "/";
			var fso= new ActiveXObject("Scripting.FileSystemObject");
			if (!fso.FolderExists(dir0)){fso.CreateFolder (dir0);} 
			
			var openpkg = pkgobj.OpenTextFile(pkgpath);
			var pkgs = openpkg.ReadAll().split("\n");
			
			var i;
			for(i=0;i<pkgs.length;i++){
				if (pkgs[i].indexOf(ip[ip.Length-1])!=-1){
					var pkg = pkgs[i].split(":")[1].replace(/[\n]/g,"").replace(/[\r]/g,"");
					...
					var sid = oSession.id;
					var dir2 = dir1 + sid + "/";
					if (!fso.FolderExists(dir2)){fso.CreateFolder (dir2);} 
		
					oSession.SaveSession(dir2 + "Session" +".txt",true);
					//oSession.SaveRequestBody(dir2 + "RequestBody" +".txt");
					if (oSession.requestBodyBytes.length > 0){
						oSession.SaveRequestBody(dir2 + "RequestBody" +".txt");}
					//oSession.SaveRequest(dir2 + "Request" +".txt",true);
			...
			if (pkgobj.FileExists(pkgpath))
			{
				...
				for(i=0;i<pkgs.length;i++){
					if (pkgs[i].indexOf(ip[ip.Length-1])!=-1){
						...
						//oSession.SaveResponseBody(dir2 + "ResponseBody" +".txt");
						if (oSession.responseBodyBytes.length > 0){
							oSession.SaveResponseBody(dir2 + "ResponseBody" +".txt");}
						oSession.SaveMetadata(dir2 + "Metadata" +".txt");
						break;
					}
