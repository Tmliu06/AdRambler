...
class InputEvent(object):
    ...
    def from_dict(event_dict):
        ...
        elif event_type == KEY_TouchAdEvent:
            return TouchAdEvent(event_dict=event_dict)
    ...
    def get_event_str_without_state(self):
        event_str = "%s(" % self.__class__.__name__
        event_dict = self.to_dict()
        for key, value in event_dict.items():
            if key != 'event_type':
                event_str += "%s=%s," % (key, value)
    ...
    def get_random_instance(device, app):
        if not device.is_foreground(app):
            ...
            if app.get_main_activity():
                component += "/%s" % app.get_main_activity().replace('..', '.')
...
class TouchAdEvent(UIEvent):
    ...
    def __init__(self, x=None, y=None, view=None, event_dict=None):
        super().__init__()
        self.event_type = KEY_TouchAdEvent
        self.x = x
        self.y = y
        self.view = view
        self.ad_feature = view['ad_feature']
        if event_dict is not None:
            self.__dict__.update(event_dict)
