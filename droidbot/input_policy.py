...
from .input_event import InputEvent, KeyEvent, IntentEvent, TouchEvent, ManualEvent, SetTextEvent, KillAppEvent, KEY_TouchAdEvent
from .utg import UTG
...
EVENT_FLAG_START_APP_BACK = "+start_app_back"
...
POLICY_NONE = "none"
POLICY_AD_BFS = "bfs_ad"
POLICY_AD_DFS = "dfs_ad"
...
class InputPolicy(object):
    ...
    def __save_event_info(self, event):
        output_dir = self.device.output_dir
        if self.current_state:
            state_str = self.current_state.state_str
            event_str = event.get_event_str(self.current_state)
        else:
            state_str = ''
            event_str = event.get_event_str_without_state()
        if hasattr(event, 'view'):
            view_str = event.view['view_str']
        else:
            view_str = ''

        ad_feature = ''
        if event.event_type == KEY_TouchAdEvent:
            ad_feature = event.ad_feature

        traffic_dir = self.app.traffic_dir
        if traffic_dir:
        ...
        with open(output_dir + "/clicked.json", "a+", encoding='utf-8') as fc:
            fc.write(json.dumps(outputs) + '\n')
...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    def generate_event_based_on_utg(self):
        ...
        if current_state.get_app_activity_depth(self.app) < 0:
            ...
            if self.__event_trace.endswith(EVENT_FLAG_START_APP + EVENT_FLAG_STOP_APP) \
                    or self.__event_trace.endswith(EVENT_FLAG_START_APP) \
                    or self.__event_trace.endswith(EVENT_FLAG_START_APP_BACK):
                self.__num_restarts += 1
                self.logger.info("The app had been restarted %d times.", self.__num_restarts)
            else:
                self.__num_restarts = 0
            ...
            # pass (START) through
            if not self.__event_trace.endswith(EVENT_FLAG_NAVIGATE):
                if self.__num_restarts > MAX_NUM_RESTARTS:
                    # If we have tried too many times but the app is still not started, stop DroidBot
                    msg = "The app had been restarted too many times."
                    self.logger.info(msg)
                    raise InputInterruptedException("The app cannot be started.")
                elif not self.__event_trace.endswith(EVENT_FLAG_START_APP):
                    # Start the app
                    self.__event_trace += EVENT_FLAG_START_APP
                    self.logger.info("Trying to start the app...")
                    return IntentEvent(intent=start_app_intent)
                else:
                    # Sometimes BACK event can save crash
                    self.__event_trace += EVENT_FLAG_START_APP_BACK
                    return KeyEvent(name="BACK")
            else:
...
class UtgGreedyAdSearchPolicy(UtgBasedInputPolicy):
    """
    Ad-Specific DFS/BFS (according to search_method) strategy to explore UFG (new)
    """

    def __init__(self, device, app, random_input, search_method):
        super(UtgGreedyAdSearchPolicy, self).__init__(device, app, random_input)
        self.logger = logging.getLogger(self.__class__.__name__)
        self.search_method = search_method
    ...
    def generate_event_based_on_utg(self):
        ...
        # Get all possible input events
        possible_events = current_state.get_possible_input_ad_policy()

        if self.random_input:
            random.shuffle(possible_events)

        if self.search_method == POLICY_AD_DFS:
            possible_events.append(KeyEvent(name="BACK"))
        elif self.search_method == POLICY_AD_BFS:
            possible_events.insert(0, KeyEvent(name="BACK"))
        ...
        # If there is an unexplored ad event, try the ad event first
        possible_ad_events = []
        for input_event in possible_events:
            if input_event.event_type == KEY_TouchAdEvent:
                if not self.utg.is_event_explored(event=input_event, state=current_state):
                    possible_ad_events.append(input_event)

        if possible_ad_events:
            input_event = self.__choose_ad_event_with_priority(possible_ad_events)
            self.logger.info("Trying an ad event, ad lib is " + input_event.ad_feature)
            self.__event_trace += EVENT_FLAG_EXPLORE
            return input_event

        # If there is an unexplored event, try the event
        for input_event in possible_events:
            if not self.utg.is_event_explored(event=input_event, state=current_state):
                self.logger.info("Trying an unexplored event.")
                self.__event_trace += EVENT_FLAG_EXPLORE
    ...
    def __choose_ad_event_with_priority(self, possible_ad_events):
        if len(possible_ad_events) == 1:
            return possible_ad_events[-1]
        events_with_priority_2 = []
        events_with_priority_1 = []
        for event in possible_ad_events:
            event.priority = 0
            children = self.current_state.get_all_children(event.view)
            for child in children:
                child_view = self.current_state.views[child]
                if "webview" in child_view['class'].lower():
                    if child_view['bounds'] == event.view['bounds']:
                        event.priority = 2
                        events_with_priority_2.append(event)
                    else:
                        event.priority = 1
                        event.view = child_view
                        events_with_priority_1.append(event)
                    break
        if len(events_with_priority_2):
            return events_with_priority_2[-1]
        if len(events_with_priority_1):
            return events_with_priority_1[-1]
        return possible_ad_events[-1]
    ...
    def __get_nav_target(self, current_state):
        ...
        for state in reachable_states:
            ...
            if state.state_str in self.__missed_states:
                continue
            # Do not consider explored states
            if self.utg.is_state_explored_ad_policy(state):
                continue
            self.__nav_target = state
            navigation_steps = self.utg.get_navigation_steps(from_state=current_state, to_state=self.__nav_target)
            if len(navigation_steps) > 0:
