...
from .input_event import TouchEvent, LongTouchEvent, ScrollEvent, SetTextEvent, KeyEvent, TouchAdEvent
...
class DeviceState(object):
    ...
    def get_possible_input_ad_policy(self):
        """
        Get a list of possible input events for this state
        :return: list of InputEvent
        """
        ...
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'ad_feature'):
                bounds = self.views[view_id]['bounds']
                if bounds not in ad_event_bounds:
                    ad_event_bounds.append(bounds)
                    possible_events.append(TouchAdEvent(view=self.views[view_id]))
                    touch_exclude_view_ids.add(view_id)
                    touch_exclude_view_ids = touch_exclude_view_ids.union(self.get_all_children(self.views[view_id]))

        touch_ad_exclude_view_ids = copy.deepcopy(touch_exclude_view_ids)

        for view_id in enabled_view_ids:
            if view_id in touch_ad_exclude_view_ids:
                continue
            if self.__safe_dict_get(self.views[view_id], 'clickable'):
                possible_events.append(TouchEvent(view=self.views[view_id]))
                touch_exclude_view_ids.add(view_id)
