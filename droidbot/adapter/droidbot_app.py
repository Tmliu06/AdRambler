...
from ..analyser.utils import get_pkg_layer, read_xposed_log, get_ignored_pkgs, read_known_ad_libs
...
class DroidBotAppConn(Adapter):
    ...
    def __init__(self, device=None, app=None):
        ...
        self.xposed_log_path = os.path.join(app.output_dir, 'xposed.log')

        self.known_ad_libs = read_known_ad_libs()
        self.ignored_pkgs = get_ignored_pkgs(app.package_name, app.main_activity)
    ...
    def handle_message(self, message):
        ...
        if message.startswith('#'):
            with open(self.xposed_log_path, 'a+', encoding='utf-8') as flog:
                flog.write(message)
    ...
    def get_views(self):
        ...
        xposed_infos = read_xposed_log(self.xposed_log_path)
        stacks = []
        for xposed_info in xposed_infos:
            if xposed_info['pkg'] == 'com.android.webview' or xposed_info['pkg'] == self.package_name:
                stacks += xposed_info['stacks']

        for view_dict in view_list:
            real_view_class = view_dict['package']
            view_dict['ad_feature'] = self.get_ad_feature_from_classname(real_view_class, stacks)

        self.last_acc_event['view_list'] = view_list
        return view_list

    def get_ad_feature_from_classname(self, classname, stacks):

        for ad_lib in self.known_ad_libs:
            if classname.startswith(ad_lib + '.'):
                return ad_lib

        for ignored_pkg in self.ignored_pkgs:
            if classname.startswith(ignored_pkg):
                return None

        tpl = get_pkg_layer(classname.split('.'), layer=3)
        for stack in stacks:
            if stack.startswith(tpl):
                return tpl
